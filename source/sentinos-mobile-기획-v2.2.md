...
## 8. 상세 조치 영역 (변형 C) — GD 요약 + 콘솔 데이터 정합

**정보 구조 슬로건 (디자인 정본)**: **식별 → 즉시 손 → 필요할 때 깊이**  
(상세: [디자인 컨셉 §1.2~1.4](../design/sentinos-mobile-디자인-컨셉.md))

| 블록 | 이름 | 역할 |
|---|---|---|
| **Main** | 이벤트 발생 내용 | 식별 — 기존 이슈 상세 핵심 (사실) |
| **Sub 1** | 긴급조치 매뉴얼 (Runbook) | 즉시 손 — **기본 랜딩** |
| **Sub 2** | AI 분석 & 조치제안 | 깊이 1 — EVENT AI |
| **Sub 3** | 유사 이슈 조치내역 | 깊이 2 — knowledge/similar |
| **Sub 4** | AI 발생이력 분석 | 깊이 3 — HISTORY AI |

MVP 이후(V1) 본격 구현. 의미론 정본 = sentinos-console `/events` 행 확장.  
**페이지 왕복으로 Main/Sub를 쪼개지 않는다** — 상세 한 화면에 Main + 2차 내비 + Sub 패널.

### 8.1 콘솔 ↔ 모바일 섹션 매핑

| 콘솔 `/events` UI | 모바일 가이드 섹션 | 데이터 원천 |
|---|---|---|
| **진급조치 매뉴얼** | Runbook 체크리스트 | `runbook` · `runbook_step` · `runbook_rule_mapping` · API `GET /v1/api/runbooks/by-rule/{ruleId}` |
| **AI 분석** | AI 분석 블록 | `event_ai_analysis` (EVENT/HISTORY, status DONE) |
| **유사이슈 조치이력** | 유사 이력 카드 + [이 조치 재사용] | `incident_knowledge` · API `GET /v1/api/knowledge/similar` (rule 정확 + pgvector, 임계 0.45) |

- 매뉴얼·유사 이력·AI **어느 것도 없으면** 상세의 AI 스트립·가이드 진입을 **숨긴다** (빈 가이드 금지).
- step_type: `CHECK` · `ACTION` · `CONTACT` · `NOTE` (콘솔 Runbook 정본). ACTION 체크 = “콘솔에서 수행함”(원격 실행 UI 없음).
- 유사 이력 status: `RAW` 재사용 가중 하향 · `미상(초동)`/원인 미확정은 재사용 버튼 없음 · `CURATED`/`VERIFIED` 우선 노출.
- AI 필드: `suspected` / `checks` / `actions` / `evidence` — **근거(evidence·유사 지식 링크) 없는 추정 항목은 렌더 금지**.
...
### 8.3 시안 샘플 (실 개발 DB, 2026-07)

| 항목 | 값 |
|---|---|
| 이벤트 | `[AUT] 승인 5xx 오류율 급증` · rule `afqs5s6xaaghsc` · severity critical |
| 진급조치 매뉴얼 | Runbook **#18** `서비스 5xx 오류율 급증 대응` (CHECK×2 · CONTACT · ACTION×2) |
| 유사이슈 | knowledge #49 CURATED (배포 검증 부하, MTTR 7분) · #47 RAW (신규 버전 /error) |
| AI | `event_ai_analysis` kind=EVENT · model `qwen2.5:7b-instruct` · 유사 92%/86% 근거 문구 |

시안: [mockups.html#m-guide](https://bdk-choi.github.io/sentinos-mobile-docs/mockups.html#m-guide)
